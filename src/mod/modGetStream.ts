...
// import FormData from 'form-data'; // Remove this line for browser compatibility
  ...
  try {
    ...
    try {
      ...
      const resumeBotDownload = await fetch(
        ...
        {
          method: 'POST',
          body: resumeBotBody as BodyInit,
          headers: {
            Referer: resumeBot,
    ...
    try {
      ...
      const instantLinkRes = await fetch(videoSeedUrl, {
        method: 'POST',
        body: InstantFromData as BodyInit,
        headers: {
...
export async function modExtractor(url: string) {
  try {
    ...
    const res = await fetch(url.split('?')[0], {
      method: 'POST',
      body: bodyFormData0 as BodyInit,
    });
    ...
    let wpHttp2Str = '';
    if (Array.isArray(wpHttp2)) {
      wpHttp2Str = wpHttp2[0] || '';
    } else if (typeof wpHttp2 === 'string') {
      wpHttp2Str = wpHttp2;
    }
    bodyFormData.append('_wp_http2', wpHttp2Str);
    const formUrl1 = $('form').attr('action');
    const formUrl = formUrl1 || url.split('?')[0];

    const res2 = await fetch(formUrl, {
      method: 'POST',
      body: bodyFormData as BodyInit,
    });
